...
제한된 웹 실험판 채택. 로컬 기본 드로잉·복구 gate와 CI/Pages 배포 gate를 통과했다.
공개 URL의 WebGPU 초기 화면까지 확인했으며 실제 펜·장시간 작업 gate는 남아 있다.
...
Linux CI·Pages 배포와 Windows CI가 통과했다. 공개 홈페이지의 Windows 다운로드 연결,
공개 웹 편집기의 WebGPU 시작, WASM MIME/header와 개인 경로 패턴 부재를 확인했다.
로컬에서 검증한 모든 편집·복구 시나리오를 공개 출처에서 다시 수행한 것은 아니다.
실행 링크와 산출물 hash는 [웹 지원 문서](../web-support.md)에 기록한다.
