...
현재 작업 묶음과 그다음 결정은 [현재 우선순위](current-priorities.md)를 먼저 본다.
웹 편집 연결·새 그림/최근 그림을 검증하고 출하하는 단계이며, 아래 Sprint 0~9는
초기 분류 기록이다. 이미 공개된 웹/Windows 알파를 미래 기능으로 재해석하지 않는다.
...
다음은 기준 그림의 실제 창·펜 확인이다. W5/고급 기능을 자동으로 시작하지 않는다.
[기능 완성 우선 계획](functionality-next.md)의 F1~F3a는 구현 기록이며 F3b/병합을 자동 진행하지 않는다.
기존 Sprint 4/5의 기본 브러시·선택·채색 필수 부분은 앞으로 당긴다. 고급 표현·대형 프로젝트
최적화와 구분하며 저장·입력 안전성 gate는 유지한다. 아래 장기 로드맵은 기능 분류 참고다.
