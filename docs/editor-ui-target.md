...
| 선택 픽셀 이동(화살표) 드래그 | 활성 레이어의 선택 픽셀 이동; 화면 팬과 별개 |
...
| 순서 | 도구 | 단축키 | 초기 구현 단계 |
|---|---|---|---|
| 1 | 화면 이동 → 선택 픽셀 이동 → 마법봉 → 올가미 → 사각형 선택 | `G` | 화면 이동 Sprint 1, 선택 계열 Sprint 2 |
...
현재 선택 마스크는 출력 페이지 안으로 제한되지만 선택 픽셀 이동 결과는 페이지 밖에도
보존된다. 이동 중 안내선을 보여주고 손을 떼면 기존 변형 경로로 확정하며 선택을 해제한다.
픽셀이 따라오는 실시간 이동 미리보기와 페이지 밖 선택 마스크는 후속 범위다.
...
크기·불투명도·경도·선 보정을 두고 수치 입력은 오른쪽 정렬한다. 크기/불투명도의
필압 적용과 최소값은 기본 닫힘 `추가 옵션`에서 조절한다.
...
두 색은 세션 범위다. 스포이트는 왼쪽 별도 그룹의 `I` 도구이며 놓는 픽셀에서 읽는다.
...
복사하며, 붙여넣기는 원본 위의 새 래스터로 즉시 확정한다. NyatiDraw 복사본은 위치·픽셀을
보존하고 외부 PNG는 원점에 둔다. 기존 선택은 해제되며, 붙인 요소의 floating 상태와
확정/취소·연속 이동은 후속 구현이다. DIB만 제공하는 외부 클립보드는 아직 읽지 않는다.
...
[Essentials E1~E3](sprints/essentials-goal.md)가 최신이다. 레이어 이름 아래 `α`는 투명도
잠금, `↳`는 하위 클리핑, Normal/Multiply 선택은 합성 모드다. 도킹 핸들에는 넣지 않는다.
