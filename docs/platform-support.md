...
| wasm/web | 제한된 웹 실험판, 배포 gate 진행 중 | 별도 승인한 Dioxus Web 호스트·공용 CPU 브러시/타일·wgpu WebGPU 합성·IndexedDB 복구 구현. Windows의 Codex 내장 브라우저에서 그리기·Undo/Redo·탭 재개·단일 편집 탭을 확인했다. 실제 펜·모바일·전체 브라우저 프로세스 재시작·device loss·성능 acceptance는 미검증이다. [범위와 실행](web-support.md) |
