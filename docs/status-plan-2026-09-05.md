...
(ADR-0034). 이후 WebView2 초기화 오류가 반복되어 **현재 최우선은 정상 GUI 시작·재시작
복구**이며 원인은 아직 확정하지 않았다. 프로필 경로 변경 시도는 효과가 없어 되돌렸다.
그 뒤 Explorer 차이와 history worker 비용을 조사한다. 아래 최초 점검 표의 미완료
