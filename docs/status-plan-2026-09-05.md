...
현재 gate와 다음 실행 순서는 [2026-09-06 점검](status-plan-2026-09-06.md)을 따른다.
이 문서는 최초 점검과 후속 개발 이력을 보존한다.

**2026-09-06 재개:** 페이지/레이어 전체 복원과 history CPU 버퍼 재사용을 검증했다
(ADR-0031/0032). PNG Open With 등록을 보완하고 갱신·충돌 거부·제거·재설치의
소유권 보존을 확인했으나, 실제 탐색기의 후보 표시와 직접 열기는 아직 미완료다
(ADR-0033). Undo worker 접수→복원 frame present API p95는 155.647ms로 확인했다
(ADR-0034). 이후 반복된 WebView2 종료 오류는 초기 `MoveFocus` 실패를 앱 전체 실패로
처리하는 경로로 좁혔다. Wry #1799를 현재 버전에 backport했고, 원본 실패/수정본 성공의
동일 조건 재현과 설치 앱의 4K Undo/Redo·저장·종료·전체 재열기 비교를 통과했다
(ADR-0035). 프로필 변경 시도는 되돌린 상태다. 두 번째 설치 앱 재시작도 present까지
진행했으나 컴퓨터 사용 도구의 foreground PID 오류로 추가 UI 확인이 중단됐다.
저장소의 같은 타일 객체 중복 읽기를 제거해 국소 tile-load p95 90.378→12.776ms를
확인했다. 핵심 손상 거부·workspace 검사와 두 scratch의 전체 재열기 비교를 통과했다
(ADR-0036). 첫 저장 반영 204.677ms 이상치는 보존했다. **다음 우선순위는 새 설치판의
Undo 전체 측정, 저장 반영 꼬리 지연, Explorer 차이**다. 아래 최초 점검 표의 미완료
항목은 후속 완료 기록과 함께 읽는다.
...
같은 설치판에서 crop 2×2→Undo 원본 16×16→재시작→Redo 2×2를 확인했다.
이어 작품 레이어 삭제→Save/재시작→Undo 복원→Save/재시작을 완료했다.
각 단계에서 모든 signed/locked/hidden tile, tree, page/PPI, snapshot/history와
독립 PNG oracle이 일치했다. 페이지 변경과 layer 삭제/복원 모두 타일 유지
