//! Bounded gesture capture. Only a completed gesture becomes a command.
...
impl EditGesture {
    ...
    pub fn push(&mut self, sample: StylusSample) {
    ...
    #[must_use]
    pub fn preview(&self) -> (&[[i32; 2]], bool) {
        if self.error.is_some() {
            return (&[], false);
        }
        (
    ...
    #[must_use]
    pub fn picker_point(&self, sample: StylusSample) -> Option<[i32; 2]> {
    ...
    /// # Errors
    /// Rejects cancelled, discontinuous, out-of-range or incomplete gestures.
    pub fn finish(mut self, sample: StylusSample) -> Result<EditCommand, String> {
