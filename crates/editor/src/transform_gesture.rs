...
impl TransformGesture {
    ...
    /// and body translation. The host must separately pin the viewport
    ...
    /// # Errors
    /// Rejects non-finite coordinates, overflowing offsets and crossed scale pivots.
    pub fn update(&self, point: [f64; 2]) -> Result<AffineTransform, String> {
        if !point.iter().all(|v| v.is_finite()) {
...
/// One open polyline for the shared guide overlay. It retraces short
...
#[must_use]
pub fn guide(projection: &TransformProjection, radius: f64) -> Vec<[f64; 2]> {
