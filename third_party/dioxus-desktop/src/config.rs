...
pub struct Config {
    ...
    pub(crate) exit_guard: Option<Box<dyn FnMut() -> bool>>,
...
impl Config {
    ...
    /// Defer event-loop exit while an application-owned native worker retires.
    /// Called on the window thread; must never block or join a live worker.
    /// Returning false keeps native messages pumping and retries after 10 ms.
    /// Also runs before resuming a panic raised by application event dispatch.
    pub fn with_exit_guard(mut self, guard: impl FnMut() -> bool + 'static) -> Self {
        self.exit_guard = Some(Box::new(guard));
        self
    }
