...
// NyatiDraw temporary, opt-in startup diagnosis. Never log IPC contents or keys.
pub(crate) fn startup_diagnostics() -> bool {
    static ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ENABLED.get_or_init(|| std::env::var_os("NAYATI_STARTUP_DIAGNOSTICS").is_some())
}
...
impl WryQueue {
    ...
    pub(crate) fn send_edits(&self) {
        ...
        if startup_diagnostics() && myself.startup_stage == 0 {
            println!(
                "desktop-startup event=first-edits-queued bytes={}",
                serialized_edits.len()
            );
            myself.startup_stage = 1;
        }
    ...
    pub(crate) fn poll_edits_flushed(
        ...
        let result = if let Some(receiver) = self_mut.edits_in_progress.as_mut() {
            receiver.poll_unpin(cx)
        } else {
            std::task::Poll::Ready(Ok(()))
        };
        if self_mut.startup_stage == 1 && result.is_ready() {
            println!(
                "desktop-startup event=first-edits-poll-ready ack_receiver_ok={}",
                matches!(result, std::task::Poll::Ready(Ok(())))
            );
            self_mut.startup_stage = 2;
        }
        result.map(|_| ())
    }
...
impl EditWebsocket {
    ...
    fn handle_connection(
        ...
        std::thread::spawn(move || {
            ...
            let mut first_edits = startup_diagnostics();
            ...
            'connection: while let Ok(msg) = edits_incoming_rx.recv() {
                ...
                if let Err(e) = websocket.send(tungstenite::Message::Binary(data.into())) {
                    if first_edits {
                        println!("desktop-startup event=first-edits-socket-send-failed");
                    }
                ...
                if first_edits {
                    println!("desktop-startup event=first-edits-socket-sent");
                }
                ...
                while let Ok(ws_msg) = websocket.read() {
                    match ws_msg {
                        ...
                        tungstenite::Message::Close(_) => {
                            if first_edits {
                                println!(
                                    "desktop-startup event=first-edits-socket-closed-before-ack"
                                );
                            }
                            break 'connection;
                        }
                ...
                if first_edits {
                    println!("desktop-startup event=first-edits-socket-read-finished acknowledged={acknowledged}");
                    first_edits = false;
                }
