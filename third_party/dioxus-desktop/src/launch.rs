...
pub fn launch_virtual_dom_blocking(virtual_dom: VirtualDom, mut desktop_config: Config) -> ! {
    ...
    let mut exit_guard = desktop_config.exit_guard.take();
    let mut pending_panic = None;
    let mut pending_exit = None;
    let (event_loop, mut app) = App::new(desktop_config, virtual_dom);

    event_loop.run(move |window_event, event_loop, control_flow| {
        // NyatiDraw local lifecycle patch: an exiting framework must continue
        // pumping native messages until cross-thread DXGI calls retire. Once
        // dispatch panics, do not reenter potentially inconsistent App state.
        if pending_panic.is_none()
            && (pending_exit.is_none() || matches!(window_event, Event::LoopDestroyed))
        {
                ...
                match window_event {
                    ...
                    Event::UserEvent(event) => match event {
                        ...
                        UserWindowEvent::Ipc { id, msg } => match msg.method() {
                            ...
                            IpcMethod::Other("nyatidraw_startup_edits") => {
                                app.handle_startup_edits(msg, id)
                            }
                            IpcMethod::Other(_) => {}
                        },
                    },
            ...
            if exit_guard.is_some()
                && matches!(
                    app.control_flow,
                    tao::event_loop::ControlFlow::ExitWithCode(_)
                )
            {
                pending_exit = Some(app.control_flow);
            }
        }
        if pending_panic.is_some() || pending_exit.is_some() {
            if exit_guard.as_mut().is_some_and(|guard| !guard()) {
                *control_flow = tao::event_loop::ControlFlow::WaitUntil(
                    std::time::Instant::now() + std::time::Duration::from_millis(10),
                );
                return;
            }
            if let Some(payload) = pending_panic.take() {
                std::panic::resume_unwind(payload);
            }
        }
        *control_flow = pending_exit.unwrap_or(app.control_flow);
    })
}
